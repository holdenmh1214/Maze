...
public class Main {
    ...
    static ArrayList<Room> possibleNeighbors(ArrayList<ArrayList<Room>> rooms, int row, int col){
      ...
      if (row > 0) neighbors.add(rooms.get(row-1).get(col)); //going up
      if (row < SIZE-1) neighbors.add(rooms.get(row+1).get(col)); //going down
      if (col > 0) neighbors.add(rooms.get(row).get(col-1)); //going left
      if (col < SIZE-1) neighbors.add(rooms.get(row).get(col+1)); //going right
        return neighbors;
    }
